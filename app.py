...
st.text("This app is created using Data Available on Kaggle. The prediction is based on Data available.")
...
cpu=st.selectbox("Prcessor",df['Cpu'].unique(),index=0)
...
hdd=st.radio("Hard Drive capicity(in GB)[If SDD set HDD 0] ?",[0,128,256,512,1000,2000],horizontal=True)
sdd=st.radio("SSD capicity(in GB) ?",[0,8,16,32,64,128,256,512,1000,2000],horizontal=True,index=7)
...
if st.button("Predict Price"):
    ...
    query=np.array([[company,typename,cpu,ram,gpu,os,weight,ips,touchscreen,cpu_speed,hdd,sdd,ppi]])
    op=pipe.predict(query)
    f_op=int(round(op[0],-2))
    st.subheader("The Price of the above Configration Laptop is ₹"+str(f_op))
